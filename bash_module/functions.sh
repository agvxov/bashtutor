function description() {
    echo "Functions are a special type of program that you can define right inside bash." \
         ...
         "However, they take arguments just like scripts. Take this for example:"
    echo "  $ sum() {"
    echo '  >   echo "$1 + $2 = $(($1 + $2))'
    echo "  > }"
    echo ""
    echo "Note: while most commands you run terminate at the first new line," \
    ...
    echo ""
    echo "Inside functions you get access to a special program: 'return'." \
         "It can be used to make specify the functions exit value by passing it as an argument."
